...
export async function register(input: {
  ...
  // pass_plain salvata per la funzione "password dimenticata" (no email).
  const r = await query<UserRow>(
    `INSERT INTO users (nick, username, pass_hash, pass_plain, is_guest)
     VALUES ($1, $1, $2, $3, false) RETURNING *`,
    [username, hash, input.password],
  );
  const user = r.rows[0]!;
  return { token: signToken(user.id), user: toPublic(user) };
}

/** Recupero password "a video": ritorna la password in chiaro per un nome utente. */
export async function recoverPassword(input: { username: string }): Promise<{ password: string }> {
  const username = input.username?.trim();
  if (!username) throw new AppError(400, 'Inserisci il nome utente');
  const r = await query<{ pass_plain: string | null }>(
    'SELECT pass_plain FROM users WHERE lower(username) = lower($1)',
    [username],
  );
  const row = r.rows[0];
  if (!row || !row.pass_plain) throw new AppError(404, 'Nome utente non trovato');
  return { password: row.pass_plain };
}
